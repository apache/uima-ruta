...
public class TypeFromStringFunctionExtension implements IRutaTypeFunctionExtension {
  ...
  private final String[] knownExtensions = new String[] { "typeFromString" };

  private final Class<?>[] extensions = new Class[] { TypeFromStringFunction.class };

  public String verbalize(RutaElement element, RutaVerbalizer verbalizer) {
    if (element instanceof TypeFromStringFunction) {
      return verbalizeName(element) + "("
              + verbalizer.verbalize(((TypeFromStringFunction) element).getExpr()) + ")";
    } else {
  ...
  public TypeFunctionExpression createTypeFunction(String name, List<RutaExpression> args)
    ...
    return new TypeFromStringFunction((AbstractStringExpression) args.get(0));
  }
